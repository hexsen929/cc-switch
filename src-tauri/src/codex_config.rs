...
pub fn write_codex_live_for_provider(
    ...
    if plan.write_full_auth {
        return write_codex_live_atomic_with_stable_provider(auth, plan.config_text.as_deref());
    }
    write_codex_live_config_atomic_with_stable_provider(plan.config_text.as_deref())?;
...
"#;
        ...
        let normalized = normalize_codex_live_config_model_provider(target).unwrap();
        let result =
