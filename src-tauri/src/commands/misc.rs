...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/hexsen929/cc-switch/releases/latest",
            None::<String>,
        )
...
fn build_lifecycle_execution_path() -> Option<std::ffi::OsString> {
    ...
    let fallback_path = std::env::join_paths(paths).ok()?;

    // 上游登录 shell 的 PATH 最接近 CLI 探测和用户终端的真实环境；fork 的目录扫描
    // 仍作为兜底，覆盖未写入 shell rc 的 nvm/fnm/mise/Volta/Homebrew 安装。
    if let Some(login_path) = login_shell_path() {
        let fallback = fallback_path.to_string_lossy();
        return Some(std::ffi::OsString::from(merge_path_segments(
            &login_path,
            &fallback,
        )));
    }
...
///
/// 只用于 `Command::env("PATH", ...)`，不会写入系统环境或 shell rc 文件。登录 shell
/// PATH 由 `build_lifecycle_execution_path` 置于最前；这里的继承 PATH 与常见 GUI 缺失目录
/// 只作为追加兜底，让 `#!/usr/bin/env node` 这类 shebang 能找到 runtime。
#[cfg(not(target_os = "windows"))]
