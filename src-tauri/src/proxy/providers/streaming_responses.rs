...
>(
    ...
    backfill_anthropic_message_start_usage(ordered_stream)
}
...
fn backfill_anthropic_message_start_usage(
    stream: impl Stream<Item = Result<Bytes, std::io::Error>> + Send + 'static,
) -> impl Stream<Item = Result<Bytes, std::io::Error>> + Send {
    // This wrapper buffers all Anthropic SSE events, then back-fills
