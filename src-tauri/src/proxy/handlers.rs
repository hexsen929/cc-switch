...
async fn handle_claude_transform(
    ...
    let transform_result = match (direct_anthropic_response, upstream_response) {
        ...
        (None, Some(response)) if api_format == "openai_chat_virtual_tools" => {
            let response =
                super::providers::tool_virtual::apply_virtual_tool_response_to_openai_chat_response(
                    response,
                );
            transform::openai_to_anthropic(response)
        }
...
pub async fn handle_alpha_search(
        ...
        .await
    {
        ...
        Err(mut err) => {
            ...
            return build_codex_proxy_error_response(&ctx, &endpoint, &err.error, false);
        }
    };
...
mod tests {
    use super::{
        body_looks_like_sse, build_codex_proxy_error_sse_body, chat_sse_to_response_value,
        classify_body_for_diagnostics, codex_proxy_error_json,
