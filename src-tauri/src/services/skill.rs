...
impl SkillService {
            ...
            if same_repo {
                ...
                if let Err(error) = sync_result {
                    if let Err(rollback_error) = db.save_skill(existing) {
                        log::error!(
                            "Failed to restore Skill {} after sync error: {rollback_error}",
                            existing.id
                        );
                    }
                    return Err(error);
    ...
    /// 在供应商切换事务尚未提交 current provider 时，按显式目标供应商同步 Skills。
    pub(crate) fn sync_to_app_for_provider(
        db: &Arc<Database>,
        app: &AppType,
        ...
        if app_dir.exists() {
            for entry in fs::read_dir(&app_dir)? {
                ...
                if let Some(skill) = indexed_skills.get(&dir_name.to_lowercase()) {
                    if !Self::is_effectively_enabled_for_app(skill, app, current_provider) {
