...
pub fn reapply_current_codex_official_live(state: &AppState) -> Result<bool, AppError> {
    ...
    // 代理接管期间以 backup/占位符为所有权信号。关闭统一会话时仅更新
    // 备份；开启统一会话时必须离开 Codex 官方接管路由，因为该模式要求
    // live 立即使用共享的 `custom` model_provider 历史桶。
    let has_live_backup =
    ...
    if has_live_backup || live_taken_over {
        if crate::settings::unify_codex_session_history() {
            // The upstream native-auth route uses its own model_provider id.
            // Unified history intentionally needs the shared `custom` bucket,
            // so leave Codex takeover only while this fork-specific mode is on.
            futures::executor::block_on(
                state
                    .proxy_service
                    .switch_proxy_target(AppType::Codex.as_str(), &current_id),
            )
            .map_err(|e| AppError::Message(format!("切换 Codex 统一会话路由失败: {e}")))?;
        } else {
            futures::executor::block_on(
                state
                    .proxy_service
        ...
        with_test_home(|state, _| {
            ...
            bound.settings_config["config"] = Value::String(
                crate::codex_config::inject_codex_unified_session_bucket("")
                    .expect("inject legacy unified session route"),
            );
...
"#,
    ...
    async fn update_current_codex_provider_syncs_live_when_proxy_takeover_active() {
        ...
        let original = Provider::with_id(
            ...
            json!({
                "auth": {
                    "OPENAI_API_KEY": "provider-key-old"
                },
                "config": r#"model_provider = "p1"
model = "gpt-old"
model_instructions_file = "/tmp/old-instruction.md"
...
experimental_bearer_token = "provider-key-old"
"#,
            ),
        )
        ...
        let updated = Provider::with_id(
            ...
            json!({
                "auth": {
                    "OPENAI_API_KEY": "provider-key-updated"
                },
                "config": r#"model_provider = "p1"
model = "gpt-updated"
model_instructions_file = "/tmp/updated-instruction.md"
disable_response_storage = true
...
"#
        ...
        let backup_toml = backup_config
            .get("config")
            .and_then(Value::as_str)
            .expect("backup config");
        assert!(
            backup_toml.contains("https://updated.example/v1"),
            "restore backup should contain the edited upstream provider config"
        );

        let live_config =
            crate::codex_config::read_codex_config_text().expect("read codex live config");
        let parsed: toml::Value = toml::from_str(&live_config).expect("parse live config");
        assert_eq!(
            parsed.get("model").and_then(|v| v.as_str()),
            Some("gpt-updated"),
            "edited model should be written to live config immediately"
        );
        assert_eq!(
            parsed
                .get("model_instructions_file")
                .and_then(|v| v.as_str()),
            Some("/tmp/updated-instruction.md"),
            "edited top-level Codex fields should be written to live config immediately"
        );
        assert_eq!(
            parsed
        ...
        assert_eq!(
            provider
                ...
                .and_then(|v| v.as_str()),
            Some("provider-key-updated"),
            "ChatGPT auth mode should use the edited provider key as bearer token"
        );
        assert_eq!(
            provider
        ...
        assert_eq!(
            ...
            Some("chatgpt"),
            "ChatGPT login state should be preserved"
        );
...
impl ProviderService {
    ...
    pub fn update(
        ...
        if is_current {
            // 如果代理接管处于激活状态，并且代理服务正在运行：
            // - 不直接走普通 Live 写入逻辑
            // - 改为更新 Live 备份，并在需要时同步代理安全的 Live 配置
            let has_live_backup =
                futures::executor::block_on(state.db.get_live_backup(app_type.as_str()))
                    .ok()
                    .flatten()
    ...
    pub fn switch(state: &AppState, app_type: AppType, id: &str) -> Result<SwitchResult, AppError> {
        ...
        // Codex official account cards support native OpenAI auth passthrough
        // while takeover remains active. Other apps' official providers stay blocked.
        ...
        if should_hot_switch {
            ...
            // 代理热切换后仍需刷新 provider-bound 资源：不同 provider 的 MCP/Skill/Prompt
            // 覆盖会变；当前分支支持在接管态下安全同步这些独立段。
            sync_provider_bound_resources(state, &app_type, true)?;
    ...
    fn switch_normal(
        ...
        if use_managed_codex_transaction {
            ...
            // Codex ChatGPT-preserving mode must read the existing live auth
            // before writing the target provider. Managed-account transitions
            // use the transaction above and must keep their selected account.
            if Self::codex_chatgpt_auth_takeover_enabled(state, &app_type) {
                ...
                // Sync to live (write_gemini_live handles its security flag internally).
                Self::write_preflighted_or_current_live(
                    state,
                    &app_type,
                    provider,
    ...
    fn codex_chatgpt_auth_takeover_enabled(state: &AppState, app_type: &AppType) -> bool {
        ...
        // Managed account cards own their selected OAuth bundle. Reusing the
        // fork's native-login snapshot here would silently replace that account
        // during a later "sync current provider" operation.
        let current_is_managed =
            crate::settings::get_effective_current_provider(state.db.as_ref(), app_type)
                ...
                .and_then(|provider| Self::managed_codex_oauth_account_id(&provider))
                .is_some();
