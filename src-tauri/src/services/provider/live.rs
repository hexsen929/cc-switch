...
fn restore_live_settings_for_provider_backfill(
    ...
    // 统一会话开关注入的共享 `custom` 路由只属于 live 配置；剥离必须发生在
    // provider 配置同步之前，否则同步会先移除 `requires_openai_auth` 标记，
    // 使注入表无法再被精确识别，最终把 live-only 路由固化到供应商配置。
    ...
    if restore_provider_token {
        if let Err(err) = crate::codex_config::restore_codex_provider_token_for_backfill(
            &mut settings,
            &provider.settings_config,
        ) {
            log::warn!(
                "Failed to restore Codex provider token while backfilling '{}': {err}",
                provider.id
            );
        }
    }
    ...
    // saved instructions-file list or tool-strip policy, so switch-away backfill
    // must overlay the DB values instead of erasing them.
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    {
        log::warn!("同步 Claude append instructions 失败: {error}");
        failures.push(format!("claude/append-instructions: {error}"));
    }
    ...
    for app_type in AppType::all() {
        ...
        if !matches!(app_type, AppType::ClaudeDesktop) {
            if let Err(e) =
                crate::services::prompt::PromptService::sync_to_live(state, app_type.clone())
            {
                log::warn!("同步 Prompt 到 {app_type:?} 失败: {e}");
                failures.push(format!("prompt/{}: {e}", app_type.as_str()));
            }
        }
    }
...
"#;
  ...
  "allowedTools": ["tool1"]
    ...
    #[test]
    fn codex_switch_backfill_preserves_saved_tool_strip_policy() {
        let provider = Provider::with_id(
            "custom".to_string(),
            "Custom".to_string(),
            json!({
                "auth": {},
                "config": "model_provider = \"custom\"\n",
                "codex_strip_tools": ["image_generation", "web_search"]
            }),
            None,
        );
        let live_settings = json!({
        ...
        assert_eq!(
            result.get("codex_strip_tools"),
            provider.settings_config.get("codex_strip_tools")
        );
    }
