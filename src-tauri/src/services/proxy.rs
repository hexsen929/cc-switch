...
impl ProxyService {
    ...
    fn apply_codex_takeover_fields_for_provider(
        ...
        let legacy_live_auth = if !preserve_chatgpt_auth
    ...
    fn is_codex_live_taken_over(config: &Value) -> bool {
        ...
        let has_local_proxy_base_url = config
            .get("config")
            .and_then(|v| v.as_str())
            .map(Self::codex_toml_has_local_base_url)
            .unwrap_or(false);

        has_placeholder || has_local_proxy_base_url
    }
