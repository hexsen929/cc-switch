...
impl ProxyService {
    ...
    fn apply_codex_takeover_fields_for_provider(
        ...
        // The built-in official route reuses Codex's native authorization and
        // therefore must never receive CC Switch's proxy placeholder. Third-party
        // routes continue to honor the fork's explicit ChatGPT-preservation mode.
        let official_passthrough =
            provider.is_some_and(crate::proxy::providers::is_codex_official_provider);

        let bearer_token = if preserve_chatgpt_auth && !official_passthrough {
            Self::codex_provider_bearer_token_for_toml(config)
        } else {
            None
        };

        let legacy_live_auth = if !official_passthrough
            && !preserve_chatgpt_auth
        ...
        if official_passthrough {
            // Keep the provider snapshot untouched. The config-only takeover
            // writer preserves the existing long-lived Codex auth.json.
        } else if preserve_chatgpt_auth {
            *auth = self.codex_chatgpt_auth_with_provider_auth(auth.clone())?;
        } else {
            let existing_auth = auth.clone();
            Self::strip_codex_chatgpt_auth_fields(auth);
            auth.insert("OPENAI_API_KEY".to_string(), json!(PROXY_TOKEN_PLACEHOLDER));
            if !crate::settings::preserve_codex_official_auth_on_switch() {
                for (key, value) in existing_auth
                    .into_iter()
                    .chain(legacy_live_auth.unwrap_or_default().into_iter())
                {
                    if key != "OPENAI_API_KEY" {
                        auth.entry(key).or_insert(value);
                    }
                }
            }
        }
        ...
        let mut updated_config = Self::apply_codex_proxy_toml_config_for_provider(
            config_str,
            proxy_codex_base_url,
            provider,
        )?;
        if preserve_chatgpt_auth && !official_passthrough {
    ...
    async fn current_codex_provider_settings_for_live(&self) -> Result<(Provider, Value), String> {
        ...
        Ok((provider, effective_settings))
    }

    async fn build_codex_takeover_provider_and_config_from_current_provider(
        &self,
        proxy_codex_base_url: &str,
        preserve_chatgpt_auth: bool,
    ) -> Result<(Provider, Value), String> {
        let (provider, mut effective_settings) =
            self.current_codex_provider_settings_for_live().await?;
        self.apply_codex_takeover_fields_for_provider(
    ...
    fn is_codex_chatgpt_auth_field(key: &str) -> bool {
        matches!(
            key,
            "OPENAI_API_KEY"
                | "auth_mode"
    ...
    fn strip_codex_chatgpt_auth_fields(auth: &mut Map<String, Value>) {
        let keys = auth
            .keys()
            .filter(|key| Self::is_codex_chatgpt_auth_field(key.as_str()))
            .cloned()
            .collect::<Vec<_>>();
        for key in keys {
            auth.remove(&key);
    ...
    fn merge_codex_provider_auth_fields(
        chatgpt_auth: Map<String, Value>,
        provider_auth: Map<String, Value>,
    ) -> Map<String, Value> {
        let mut merged_auth = chatgpt_auth
            .into_iter()
            .filter(|(key, _)| Self::is_codex_chatgpt_auth_field(key.as_str()))
            .collect::<Map<String, Value>>();

        for (key, value) in provider_auth {
            if !Self::is_codex_chatgpt_auth_field(key.as_str()) {
    ...
    fn codex_chatgpt_auth_with_provider_auth(
        &self,
        provider_auth: Map<String, Value>,
    ) -> Result<Map<String, Value>, String> {
        let chatgpt_auth = self
            .normalize_and_store_codex_chatgpt_auth(provider_auth.clone())
            .or_else(|_| self.load_codex_chatgpt_auth_for_takeover())?;
        Ok(Self::merge_codex_provider_auth_fields(
            chatgpt_auth,
            provider_auth,
        ))
    }
    ...
    fn apply_codex_chatgpt_direct_fields(
        ...
        let provider_auth = root
            ...
            .cloned()
            .unwrap_or_default();
        let merged_auth = self.codex_chatgpt_auth_with_provider_auth(provider_auth)?;
        ...
        if enabled {
            ...
            if backup_from_current_provider {
                ...
                self.update_live_backup_from_provider_inner(app_type_str, &provider, None)
                    .await?;
            } else if restore_existing_backup_before_takeover {
    ...
    fn codex_live_has_proxy_placeholder(config: &Value) -> bool {
        let has_proxy_token_in_auth = config
            .get("auth")
        ...
        let has_proxy_token_in_config = config_text
            .and_then(crate::codex_config::extract_codex_experimental_bearer_token)
            .as_deref()
            == Some(PROXY_TOKEN_PLACEHOLDER);

        has_proxy_token_in_auth || has_proxy_token_in_config
    }

    fn is_codex_live_taken_over(config: &Value) -> bool {
        let has_placeholder = Self::codex_live_has_proxy_placeholder(config);
        let config_text = config.get("config").and_then(Value::as_str);
        let has_local_proxy_base_url = config_text.is_some_and(Self::codex_toml_has_local_base_url);
        let has_official_proxy_route =
            config_text.is_some_and(crate::codex_config::codex_config_has_official_proxy_route);

        has_placeholder || has_local_proxy_base_url || has_official_proxy_route
    }
    ...
    /// bucket. The upstream official takeover route has a dedicated provider id,
    /// so this fork-specific mode leaves Codex takeover before writing Official.
    async fn leave_codex_takeover_for_unified_official_provider(
        &self,
        provider: &Provider,
        had_backup: bool,
        live_taken_over: bool,
        ...
        let effective_settings = build_effective_provider_for_live_with_codex_oauth_manager(
            self.db.as_ref(),
            &app_type,
            provider,
            &self.codex_oauth_manager,
    ...
    pub(crate) async fn hot_switch_provider_inner(
        ...
        let previous_codex_live_state = if matches!(app_type_enum, AppType::Codex)
            && (should_sync_backup || leave_codex_takeover)
        {
            Some(
                crate::codex_config::CodexLiveStateSnapshot::capture()
    ...
    fn merge_codex_common_config_sections_from_existing_config(
        target_config: &str,
        existing_config: &str,
        ...
        for key in [
            ...
            if let Some(existing_item) = existing_doc.get(key) {
                if target_doc.get(key).is_none() {
                    target_doc[key] = existing_item.clone();
                }
            }
        }
    ...
    fn update_toml_requires_openai_auth(toml_str: &str, enabled: bool) -> String {
        ...
        if let Some(provider_key) = provider_key {
            ...
            if let Some(model_providers) = doc["model_providers"].as_table_mut() {
                ...
                if let Some(provider_table) = model_providers[&provider_key].as_table_mut() {
                    provider_table["requires_openai_auth"] = toml_edit::value(enabled);
                    return doc.to_string();
                }
            }
        }

        doc["requires_openai_auth"] = toml_edit::value(enabled);
        doc.to_string()
    }
            ...
            if crate::settings::preserve_codex_official_auth_on_switch() {
                if let (Some(auth), Some(config_str)) = (
                        ...
                        == Some(PROXY_TOKEN_PLACEHOLDER)
                    {
                        ...
                        .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                        crate::codex_config::write_codex_live_config_atomic_with_stable_provider(
                            Some(&live_config),
                        )
                        .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                        return Ok(());
    ...
    fn write_codex_takeover_live_for_provider(
        ...
        let preserve_mode = crate::settings::preserve_codex_official_auth_on_switch();

        // Upstream's built-in official route always reuses Codex's native auth.
        // When the legacy global preservation switch is disabled, retain the
        // upstream config-only behavior for third-party placeholder routes too.
        // If the global switch is enabled, the per-app toggle remains authoritative:
        // turning it off deliberately writes API/placeholder auth mode.
        if official_passthrough || (!preserve_mode && placeholder_auth) {
        ...
        if preserve_mode {
            if let Some(auth) = config.get("auth") {
                ...
                let prepared_config =
                    ...
                    .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                let live_config = if Self::codex_auth_has_proxy_placeholder(auth) {
                    let token = crate::codex_config::extract_codex_auth_api_key(auth);
                    if token.as_deref() == Some(PROXY_TOKEN_PLACEHOLDER) {
                        Self::update_toml_experimental_bearer_token(
                            &prepared_config,
                            PROXY_TOKEN_PLACEHOLDER,
                        )
                    } else {
                ...
                } else if Self::codex_config_has_proxy_placeholder(config) {
                    prepared_config
                } else {
    ...
    fn write_codex_live_verbatim_with_auth_guard(
        ...
        use crate::codex_config::{get_codex_auth_path, write_codex_config_text};
...
mod tests {
    ...
    use crate::provider::{
        AuthBinding, AuthBindingSource, ClaudeAppendInstructionsConfig, ProviderMeta,
        ProviderPromptOverrideMode, ProviderPromptOverrides, ProviderResourceOverrides,
...
"#
    ...
    async fn codex_takeover_preserves_oauth_auth_json_when_preserve_enabled() {
        ...
        service
            .start()
            .await
            .expect("start proxy server with ephemeral port");
        let oauth_auth = json!({
            "auth_mode": "chatgpt",
            "tokens": {
                "id_token": "oauth-id",
...
"#
        ...
        assert!(
            ...
            "takeover should use the current resolved proxy base_url"
        );
        assert!(
            live_config.contains(PROXY_TOKEN_PLACEHOLDER),
    ...
    fn codex_chatgpt_takeover_preserves_oauth_fields_and_nulls_api_key() {
        ...
        let mut live_config = json!({
            "auth": {
                ...
                "provider_custom_auth": "keep-me",
                "provider_nested_auth": { "keep": true }
            },
            "config": r#"
...
experimental_bearer_token = "router-token"
...
"#
        ...
        assert_eq!(
            parsed
                ...
                .and_then(|v| v.as_str()),
            Some("/tmp/provider-instructions.md")
        );
        assert_eq!(
            parsed
    ...
    #[test]
    #[serial]
    fn codex_chatgpt_auth_merge_keeps_only_current_provider_custom_auth_fields() {
        let chatgpt_auth = json!({
            ...
            "old_provider_auth": "must-not-leak"
        })
        .as_object()
        .expect("chatgpt auth object")
        .clone();
        let provider_auth = json!({
            "OPENAI_API_KEY": "provider-key",
            "current_provider_auth": "keep-me"
        })
        .as_object()
        .expect("provider auth object")
        .clone();

        let merged = ProxyService::merge_codex_provider_auth_fields(chatgpt_auth, provider_auth);

        assert_eq!(
            merged.get("current_provider_auth").and_then(Value::as_str),
            Some("keep-me")
        );
        assert!(
            merged.get("old_provider_auth").is_none(),
            "custom auth fields from a previous provider snapshot must not leak"
        );
        assert!(merged.get("OPENAI_API_KEY").is_some_and(Value::is_null));
        assert_eq!(
...
model_reasoning_effort = "high"
model_instructions_file = "/tmp/current-provider-instructions.md"
disable_response_storage = true
...
"#
        ...
        assert!(
            ...
            "invalid ChatGPT fields should be stripped when default route mode is used"
        );
        assert_ne!(
            auth.get("auth_mode").and_then(Value::as_str),
            Some("chatgpt")
...
"#
        ...
        assert_eq!(
            ...
            "live ChatGPT OAuth auth must remain untouched while the proxy route is repaired"
        );
