...
impl ProxyService {
    ...
    async fn current_codex_provider_settings_for_live(&self) -> Result<Value, String> {
    ...
    async fn build_codex_takeover_config_from_current_provider(
        ...
        let mut effective_settings = self.current_codex_provider_settings_for_live().await?;
    ...
    /// Rebuild Codex takeover live config from the effective current provider.
    ///
    /// Startup can restore the proxy service before any user action. In that case
    /// the live config must be refreshed from the current provider immediately,
    /// otherwise Codex may send its first request with restored/stale live files
    /// until the user manually switches providers.
    pub async fn sync_codex_takeover_to_current_provider(&self) -> Result<(), String> {
        ...
        if has_backup || live_taken_over {
            self.update_live_backup_from_provider_inner("codex", &provider)
                .await?;
        }

        self.takeover_live_config_strict(&AppType::Codex).await?;
        self.sync_active_target_for_app(&AppType::Codex).await?;
        Ok(())
    }
...
"#
    ...
    #[tokio::test]
    #[serial]
    async fn startup_codex_takeover_sync_rewrites_live_from_current_provider() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
...
"#
            ...
            .expect("seed stale taken-over live config");
        ...
        service
            ...
            .await
            .expect("startup sync should rebuild codex takeover config");
        ...
        assert_eq!(
            stored
                ...
                .and_then(Value::as_str),
            Some("current-key"),
            "startup sync should refresh restore backup to current provider auth"
        );
        ...
        assert_eq!(
            live.get("auth")
                ...
                .and_then(Value::as_str),
            Some(PROXY_TOKEN_PLACEHOLDER),
            "taken-over live auth should remain proxy-managed"
        );
        ...
        assert_eq!(
            parsed_live.get("model_provider").and_then(|v| v.as_str()),
            Some("stable"),
            "startup sync should preserve the existing Codex history bucket"
        );
        let provider = parsed_live
            .get("model_providers")
            .and_then(|v| v.get("stable"))
            .expect("stable provider table");
        assert_eq!(
            provider.get("base_url").and_then(|v| v.as_str()),
            Some("http://127.0.0.1:15721/v1"),
            "live config should point Codex at the local proxy before the first message"
        );
